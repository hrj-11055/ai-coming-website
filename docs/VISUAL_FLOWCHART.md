...
graph TB
    ...
    Reports -->|定时读取| Sync[sync-reports-to-website.sh]
...
**涉及文件**:
- `sync-reports-to-website.sh` - 同步脚本
- `setup-report-sync.sh` - 设置cron
...
graph TD
    ...
    %% 自动化脚本依赖
    sync[sync-reports-to-website.sh] -->|调用| converter[html-to-json-converter.js]
    sync -->|API调用| server
    ...
    setup[setup-report-sync.sh] -->|配置| cron[cron定时任务]
    cron -->|触发| sync
...
| **sync-reports-to-website.sh** | ~114 | 自动同步脚本 | 转换器 + API + 文件系统 |
