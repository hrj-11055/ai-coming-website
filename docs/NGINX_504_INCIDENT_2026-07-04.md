...
执行时间：`2026-07-04 15:57 CST`

用户确认 Dify 已不再使用后，停用 `/opt/dify/docker` 这套 Docker Compose 服务。
...
- 使用 `stop` 而不是 `down -v`。
- Dify 容器停止，但容器和数据卷保留。
- 如果后续确实需要恢复，可在 `/opt/dify/docker` 执行 `docker-compose start`。
...
```text
停用前 available memory: 678MiB
停用后 available memory: 2.3GiB
```
...
```text
GET  https://aicoming.cn/api/health       -> 200 OK
HEAD https://aicoming.cn/                 -> 200 OK
HEAD https://aicoming.cn/admin-login.html -> 200 OK
POST https://aicoming.cn/api/auth/login   -> 400 用户名和密码不能为空
```

这说明 Dify 停用没有影响 AIcoming。
