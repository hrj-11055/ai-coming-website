...
document.addEventListener('DOMContentLoaded', async function() {
    // 检查localStorage中的数据
    const keywordData = localStorage.getItem('ai_keywords_sync_data');
    const newsData = localStorage.getItem('news_database');
...
async function loadNewsData() {
    try {
        ...
        const articles = await window.loadNewsFromAPI(params);
        renderNewsArticles(articles);
    } catch (error) {
...
async function loadNewsByDate(date) {
    try {
        ...
        const response = await fetch(`/api/news/date/${date}?${params}`);
        if (!response.ok) {
            ...
            <article class="article-item ${categoryClass}" id="article-${index}" data-date="${getDateKey(article.published_at)}">
                <div class="flex items-start justify-between mb-3">
                    ...
                    <div class="text-right">
                        <span class="text-xs text-gray-400">${article.importance_score || 5} 分</span>
                    </div>
                </div>
...
function renderOutline(articles) {
    ...
    articles.forEach((article, index) => {
        html += `
            <a href="#article-${index}" class="outline-link" onclick="highlightArticle(${index}); return false;">
