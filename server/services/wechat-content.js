...
function buildDailyNewspicImagePrompt({ date, coreItems, content }) {
    ...
    return [
        '你是一名资深中文报纸美术编辑和信息视觉设计师。',
        `请直接创作一张《小元说 AI日报》报纸图，日期为 ${formatNewspicChineseDate(date)}。不要先解释设计过程，也不要只给版式方案，直接生成最终图片。`,
        '请先理解十条新闻，自主提炼一个能统领全日内容的主标题，并根据新闻重要性自由决定版面、视觉风格和信息层级。选择少数最值得视觉化的新闻，加入与事件相关的品牌标志、产品形象、代表性物件、人物剪影或关键数字作为视觉锤；其他新闻保持简洁，让整张图有重点、有节奏，而不是十个同样大小的文字框。',
        '可以压缩和改写图中文字，但不得改变事实、虚构数字或加入正文之外的新闻。优先保证中文准确、主标题醒目、关键数字突出、手机阅读清晰。',
        '如果同时上传了参考图，只参考它的专业报纸气质、主次层级和图文结合方式，不照抄具体布局，由你完成新的设计。',
        '',
        '【今日内容】',
        contentReference,
        '',
        '基于以上内容，画一幅 AI 日报的报纸图，并附上今日日期。'
    ].join('\n');
}
